...
Re-do exercise 02 but now look at convergence: Plot the $x$ chain as a function of timestep. Also split the chain into 4 contiguous segments (the 1st, 2nd, 3rd, and 4th quarters of the chain). In each of these 7, compute the empirical mean and empirical variance of $x$. What do you conclude about convergence from these heuristics?
...
```{julia}
function autocorr(𝕩; lags=1:length(𝕩))
	lx = length(𝕩)
	z̄ = 𝕩 .- mean(𝕩)
	res = map(lags) do l # naive implementation: 𝒪(n²)
		dot(z̄[begin:(lx-l)], z̄[(1+l):end])
	end
...
```{julia}
Δₑₓ₁₁ = 1:50;
acfₑₓ₁₁ = autocorr(samplesₑₓ₀₂; lags=Δₑₓ₁₁);
plot(Δₑₓ₁₁, acfₑₓ₁₁, label=nothing, xlabel="Δ", ylabel="Cₓ(Δ)")
```
...
i read Sokal 1997 but admittedly no clue, so instead i transpile the authors’ code
...
chainsₑₓ₁₂ = Dict(
	"1st half" => samplesₑₓ₀₂[begin:(N_samples÷2)],
	"inter-quarters" => samplesₑₓ₀₂[(N_samples÷4):(3*(N_samples÷4))],
	"2nd half" => samplesₑₓ₀₂[(N_samples÷2):end],
	"full" => samplesₑₓ₀₂
);
